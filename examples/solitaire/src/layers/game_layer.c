...
static void game_loop(Zen *zen, wint_t key) {
    switch (key) {
        case L'q': case L'й':            zen_change_layer(zen, MENU_ID);     break;
        case KEY_CTRL_A:                 zen_global_move(zen, CURSOR_LEFT);  break;
        case KEY_CTRL_D:                 zen_global_move(zen, CURSOR_RIGHT); break;
    }
}
