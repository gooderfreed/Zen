...
static void init_game(Zen *zen) {
    ...
    Screen *screen = init_screen(arena, SCREEN_WIDTH, SCREEN_HEIGHT, COLOR_LIGHT_GRAY, COLOR_BLACK, ' ');
    Cursor *cursor = init_cursor(arena, NULL);
    MapLayer *game_layer = game_layer_init(arena);

    // create map
    Map *map = init_map(arena, 1, COORDS(0, 0, 0));
