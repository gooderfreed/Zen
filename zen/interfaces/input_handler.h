...
typedef struct InputHandler {
    bool (*handle_input)(void *self, Zen *zen, wint_t key);
} InputHandler;
...
static inline bool HANDLE_INPUT(void *object, Zen *zen, wint_t key) {
    return GET_INPUT_HANDLER(object)->handle_input(object, zen, key);
}
...
#define INPUT_HANDLER_FULL(arena, object, _handle_input)                                                                               \
    do {                                                                                                                               \
        ...
        if (!_handle_input)                                                                                                            \
            wprintf(L"Error in '%s': InputHandler interface is missing 'handle_input' function\n", GET_INTERFACES(object)->name);      \
    } while (0)
